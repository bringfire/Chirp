...
class TestGeneratedSolvePaths:
    ...
    def test_live_and_replay_share_one_reader_path(self):
        ...
        assert 'body = ExtractObject(frozenText, "body");' in script

    def test_missing_model_never_throws(self):
        script = _create()["script"]
        assert "throw new Exception($\"Chirp error" not in script
        assert 'catch (HttpRequestException)' not in script
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            with patch("chirp.adapter.dspy.ChainOfThought", side_effect=AssertionError("LLM path must not run")):
                with pytest.raises(ModelUnavailable, match="ANTHROPIC_API_KEY is not set"):
                    adapter.call("brief -> span", {"brief": "x"}, {"span": "float"}, category="planner", use_cache=False)
...
class TestServerContract:
    def test_call_maps_model_unavailable_to_503(self):
        with patch("chirp.server.adapter.call", side_effect=ModelUnavailable("ANTHROPIC_API_KEY is not set for anthropic/claude-opus-5")):
    ...
    def test_health_reports_model_readiness(self):
        with patch("chirp.server.adapter.model_unavailable_reason", return_value="ANTHROPIC_API_KEY is not set for anthropic/claude-opus-5"):
            data = client.get("/health").json()
        assert data["status"] == "ok"
        assert data["model_ready"] is False
        assert "ANTHROPIC_API_KEY" in data["model_reason"]
