...
def _write_frozen_helpers(w) -> None:
    ...
    w("            // RhinoCode script inputs are ScriptVariableParam; cover the generic")
    w("            // persistent-param shapes first, then fall back to reflection.")
    w("            // Store a GH_String, never a GH_ObjectWrapper: the wrapper does not write")
    w("            // its payload into the .gh file, so the snapshot would reopen empty.")
    w("            var goo = param as GH_PersistentParam<IGH_Goo>;")
    w("            if (goo != null)")
    w("            {")
    w("                goo.PersistentData.Clear();")
    w("                goo.PersistentData.Append(new GH_String(snapshot));")
    w("                RefreshPin(param);")
    w("                return;")
    w("            }")
    w("            var text = param as GH_PersistentParam<GH_String>;")
    w("            if (text != null)")
    w("            {")
    ...
    w("            object payload = new GH_String(snapshot);")
    w('            var append = data.GetType().GetMethod("Append", new[] { payload.GetType() });')
    w("            if (append == null)")
    w('                append = data.GetType().GetMethod("Append", new[] { typeof(IGH_Goo) });')
    w("            if (append == null)")
    w("            {")
    w("                foreach (var candidate in data.GetType().GetMethods())")
    w("                {")
    w('                    if (candidate.Name != "Append") continue;')
    w("                    var ps = candidate.GetParameters();")
    w("                    if (ps.Length == 1 && ps[0].ParameterType.IsInstanceOfType(payload)) { append = candidate; break; }")
    w("                }")
    w("            }")
    w("            if (append != null) { append.Invoke(data, new[] { payload }); RefreshPin(param); }")
    w('            else Print("Chirp: persistent data has no Append for the frozen result.");')
    w("        }")
    w("        catch (Exception ex)")
    w("        {")
    ...
    w("        // value. Reset and re-collect now so the next solve (and the canvas) see the")
    w("        // new snapshot. This solve already consumed its inputs, so it is unaffected,")
    w("        // and nothing is expired, so no extra solve is triggered.")
    w("        param.ClearData();")
    w("        param.CollectData();")
    w("    }")
